...
from layers import BackprojectDepth, DepthWarping
...
def compute_errors(gt_track, pred_track, mask, ratio=0.8):
    ...
    abs_rel = torch.abs(gt[final_mask] - pred[final_mask])
...
def evaluate(opt):
    ...
    gt_path = os.path.join(splits_dir, opt.eval_split, "gt_{}frames.npz".format(snippet_size))
