...
# Most of the implementations are borrowed from MonoDepth2: https://github.com/nianticlabs/monodepth2/blob/master/layers.py
...
class ResBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride, dilation=1):
        ...
        self.pad = dilation if dilation > 1 else kernel_size // 2
        self.conv = nn.Conv2d(in_channels, out_channels, kernel_size, stride, padding=self.pad, dilation=dilation)
...
def upsample(x, mode='bilinear'):
    ...
    """
    return F.interpolate(x, scale_factor=2, mode=mode, align_corners=False)
